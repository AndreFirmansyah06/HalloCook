...
var adminEmails = ["admin@hallocook.com", "indomieseleramu1@gmail.com"];
var authService = {
  async signup(userData) {
    const hashedPassword = await import_bcryptjs.default.hash(userData.password, 10);
...
var recipeService = {
  ...
  async update(id, recipe) {
    ...
    const updatePayload = {};
    if (cleanRecipe.title !== void 0) updatePayload.title = cleanRecipe.title;
    if (cleanRecipe.description !== void 0) updatePayload.description = cleanRecipe.description;
    if (cleanRecipe.image_url !== void 0) updatePayload.image_url = cleanRecipe.image_url;
    if (cleanRecipe.difficulty !== void 0) updatePayload.difficulty = cleanRecipe.difficulty;
    if (cleanRecipe.category !== void 0) updatePayload.category = cleanRecipe.category;
    if (cleanRecipe.chef_name !== void 0) updatePayload.chef_name = cleanRecipe.chef_name;
    if (cleanRecipe.duration !== void 0) updatePayload.duration = cleanRecipe.duration;
    if (cleanRecipe.servings !== void 0) updatePayload.servings = typeof cleanRecipe.servings === "number" ? cleanRecipe.servings : parseInt(cleanRecipe.servings) || 0;
    if (cleanRecipe.calories !== void 0) updatePayload.calories = typeof cleanRecipe.calories === "number" ? cleanRecipe.calories : parseInt(cleanRecipe.calories) || 0;
    if (cleanRecipe.ingredients !== void 0) updatePayload.ingredients = cleanRecipe.ingredients;
    if (cleanRecipe.steps !== void 0) updatePayload.steps = cleanRecipe.steps;
...
var userService = {
  ...
  async createAdmin(userData) {
    const hashedPassword = await import_bcryptjs2.default.hash(userData.password, 10);
    const { data: existing } = await supabase.from("users").select("id").eq("email", userData.email).single();
    if (existing) throw new Error("Email already registered");
    const { data, error } = await supabase.from("users").insert([{
      username: userData.username,
  ...
  async updateAdmin(id, userData) {
    ...
    if (updateData.password) {
      updateData.password = await import_bcryptjs2.default.hash(updateData.password, 10);
    } else {
...
var _dirname = "";
try {
  _filename = typeof __filename !== "undefined" ? __filename : (0, import_url.fileURLToPath)(null);
  _dirname = typeof __dirname !== "undefined" ? __dirname : import_path.default.dirname(_filename);
} catch (e) {
...
var isVercel = process.env.VERCEL === "1" || !!process.env.VITE_VERCEL_ENV;
if (!isVercel) {
  startServer();
} else {
  if (process.env.NODE_ENV !== "production") {
