      ...
      <div className="mx-auto max-w-7xl">
        <div className="glass flex items-center justify-between rounded-full px-6 py-3 shadow-2xl border-white/10">
          ...
          <div className="hidden md:flex items-center gap-8">
            {user?.role !== 'admin' && (
              <>
                <NavLink to="/" className={({ isActive }) => cn("text-xs font-medium uppercase tracking-widest transition-colors hover:text-brand-coral", isActive ? "text-brand-coral" : "text-white/70")}>Explore</NavLink>
                <NavLink to="/search" className={({ isActive }) => cn("text-xs font-medium uppercase tracking-widest transition-colors hover:text-brand-coral", isActive ? "text-brand-coral" : "text-white/70")}>Discover</NavLink>
                {user && (
        ...
        {isOpen && (
          ...
          >
            <div className="glass rounded-3xl p-6 shadow-2xl flex flex-col gap-4">
               <Link to="/" onClick={() => setIsOpen(false)} className="text-lg font-medium p-2 border-b border-white/10">Home</Link>
               <Link to="/search" onClick={() => setIsOpen(false)} className="text-lg font-medium p-2 border-b border-white/10">Discover</Link>
               {user && <Link to="/dashboard" onClick={() => setIsOpen(false)} className="text-lg font-medium p-2 border-b border-white/10">Dashboard</Link>}
               {user ? (
